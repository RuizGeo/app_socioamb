...
# Ler as camadas socioambientais
embargos_ibama = ler_geodataframe(gpkg_file, dict_camadas['Embargos IBAMA'])


# Acessar camandas que tenham sobreposição com o imóvel selecionado
camadas_intersect = [ k for k, v in dict_camadas.items() if gdf_incra_selecionado[v].values[0] != 0]

# Criar a lista com os nome das colunas (camadas) que tiveram sobreposição
nomes_camadas_intersect = [dict_camadas[camada] for camada in camadas_intersect]
...
st.dataframe(pd.DataFrame(gdf_incra_selecionado[columns_renamed].iloc[0]).T)#gdf_incra_selecionado[nomes_camadas_intersect].iloc[0])
